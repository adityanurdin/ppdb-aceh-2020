...
<body>
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
                        <a class="dropdown-item" href="{{route('cat.end')}}"><i class="fas fa-door-open"></i> Keluar
                            Halaman Ujian</a>
                    </div>
                </li>
            </ul>
    ...
    <div class="container mt-3">
            ...
            <div class="divss" id="Soal_{{$item->nomor_soal}}">
                <div class="row">
                    ...
                    <div class="col d-flex justify-content-end">
                        <h4 class="px-5 text-secondary"><i class="fas fa-hourglass-half"></i> Sisa Waktu <span
                ...
                <div class="row no-gutters mt-5">
                    ...
                    <div class="col-12 col-sm-6 col-md-8">
                        ...
                        <form action="" id="form_soal_{{$item->nomor_soal}}" method="POST">
                            @csrf
                            <input type="hidden" name="uuid_jawaban" id="id_jawaban{{$item->nomor_soal}}"
                                value="{{Dits::encodeDits($jawab['uuid'])}}" readonly>
                            <input type="hidden" name="nomor_soal" id="nomor_soal{{$item->nomor_soal}}"
                                value="{{Dits::encodeDits($item->nomor_soal)}}" readonly>
                            <input type="hidden" name="kode_soal" id="kode_soal{{$item->nomor_soal}}"
                                value="{{Dits::encodeDits($item->kode_soal)}}" readonly>
                            <input type="hidden" name="kode_pendaftaran" id="kode_pendaftaran{{$item->nomor_soal}}"
                                value="{{Dits::encodeDits($pendaftaran->kode_pendaftaran)}}" readonly>
                            <input type="hidden" name="nums" id="nums_{{$item->nomor_soal}}"
                                value="{{Dits::encodeDits($item->nomor_soal)}}" readonly>
                            <div class="form-group">
                                <input type="radio" name="jawaban[]" id="" value="A"
                                    {{Dits::selected($item->jawaban , 'A' , 'selected')}}
                                    onclick="SendTable('{{$item->nomor_soal}}','a')">
                                <label for="" class="label_radio">A. {{$item->a}}</label>
                            </div>
                            <div class="form-group">
                                <input type="radio" name="jawaban[]" id="" value="B"
                                    {{Dits::selected($item->jawaban , 'B' , 'selected')}}
                                    onclick="SendTable('{{$item->nomor_soal}}','b')">
                                <label for="" class="label_radio">B. {{$item->b}}</label>
                            </div>
                            <div class="form-group">
                                <input type="radio" name="jawaban[]" id="" value="C"
                                    {{Dits::selected($item->jawaban , 'D' , 'selected')}}
                                    onclick="SendTable('{{$item->nomor_soal}}','c')">
                                <label for="" class="label_radio">C. {{$item->c}}</label>
                            </div>
                            <div class="form-group">
                                <input type="radio" name="jawaban[]" id="" value="D"
                                    {{Dits::selected($item->jawaban , 'E' , 'selected')}}
                                    onclick="SendTable('{{$item->nomor_soal}}','d')">
        ...
        <div class="card card-body">
            <a title="Selesai Ujian CAT"
                onclick="ExportJawaban('frans_table');return confirm('Apakah Anda Yakin Sudah Menjawab Semua Soal?');"
                href="{{route('cat.end')}}" class="btn btn-sm btn-info float-right"><i class="fas fa-check-circle"></i>
                Selesai Ujian CAT</a> &nbsp
            <a title="Simpan Semua Jawaban" class="btn btn-sm btn-info float-right" onclick="KirimSemuaJawaban();"><i
